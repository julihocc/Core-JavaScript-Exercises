...
class DirectedGraph {
    ...
    dijkstra(startValue, endValue) {
        ...
        while (!pq.isEmpty()) {
            ...
            let currentNode = [...this.nodes.values()].find((node) => node.value === value);
            if (!currentNode) {
        ...
        for (let at = endValue; at != null; at = prev.get(at)) {
            // path.push(at);
            if (at)
                path.push(at);
        }
        ...
        `Shortest path from ${startValue} to ${endValue}: ${path.join(" -> ")} with total distance: ${distances.get(endValue)}`);
    }
