...
function solution(initArr, m, arr, used, notUsed, level, print) {
    ...
    for (let curr of arr) {
        ...
        // change the filter
        const others = initArr.filter((word) => !used.includes(word) && curr[curr.length - 1] == word[0]);
        if (print)
    ...
    if (print)
        console.log(`${"\t".repeat(level)}bestCandidate ${bestCandidate} ${bestCandidate.length}`);
    level--;
