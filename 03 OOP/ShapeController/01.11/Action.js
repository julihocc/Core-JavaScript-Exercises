export default class Action {
  ...
  async apply(isCompressed) {
    ...
    if (this.currentShape) {
      ...
      if (isCompressed) {
        let repetitions = Math.max(
          Math.abs(this.deltax),
          Math.abs(this.deltay)
        );
        console.log("Repetitions", repetitions);
        let normalizedDeltax = Math.abs(this.deltax) / repetitions;
        console.log("Normalized deltax", normalizedDeltax);
        let normalizedDeltay = Math.abs(this.deltay) / repetitions;
      ...
      // FIXME: action should't be in charge of drawing
      this.currentShape.draw();
    }
  }
  toString() {
