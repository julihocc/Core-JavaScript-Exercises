...
function reducer(state: IState, action: ActionOnContact) {
  switch (action.type) {
    case "add":
...
export function ContactContainer() {
  ...
  const handleAddContact = (newContact: IContact) => {
    const newAction: ActionOnContact = {
      type: "add" as ActionOnContactType.ADD,
      newContact,
    };
    dispatch(newAction);
  };
  ...
  const handleDeleteContact = (targetId: ID) => {
    ...
    dispatch({ type: "delete" as ActionOnContactType.DELETE, targetId });
    setDeleting(null);
  };
  ...
  const handleToggleFavorite = (id: ID) => {
    // dispatch({ type, id });
    dispatch({
      type: "toggleFavorite" as ActionOnContactType.TOGGLE_FAVORITE,
      id,
    });
  };
