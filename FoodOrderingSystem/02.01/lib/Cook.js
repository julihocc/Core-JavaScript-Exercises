export default class Cook {
    ...
    prepareOrder() {
        ...
        if (this.mediator.orders.size() > 0 && this.status === "available") {
            ...
            // debugger;
            const ingredientsAvailable = order.items.every((item) => this.mediator.requestIngredients(item.ingredients));
            if (ingredientsAvailable && !order.isTaken) {
                ...
                console.warn(`${this.name} cannot prepare the order ${order.id} because it is taken.`);
            }
            else {
                console.log(`${this.name} cannot prepare the order due to missing ingredients.`);
            }
        }
